...
open_followups = lib.get_followups("open")
week_ago = (date.today() - timedelta(days=7)).isoformat()
recent_notes = lib.get_notes(date_from=week_ago)

m1, m2, m3, m4 = st.columns(4)
m1.metric("Labs awaiting confirmation", len(pending))
m2.metric("Open follow-ups", len(open_followups), delta=f"{len(overdue)} overdue" if overdue else None,
          delta_color="inverse")
m3.metric("Notes this week", len(recent_notes))
