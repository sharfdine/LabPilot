...
This exists because of a real incident: two schema-changing fixes in one
session meant the deployed database reset twice. The **Backup & Restore**
page (💾) is the fix:

- **Download database file (.db)** — an exact copy; restoring it puts
  everything back precisely as it was.
- **Download Excel workbook** — every table as a sheet, for reading or
  handing off without needing this app at all.
- **Restore** requires typing `RESTORE` in all caps before the button
  even becomes clickable — a deliberate speed bump, since it overwrites
  everything currently in the app.
- The home page and the Backup page itself both warn you if it's been
  more than 7 days since your last backup, or if you've never taken one.

**The habit that matters**: back up right before pushing any code change
to GitHub, since every commit triggers a redeploy, and every redeploy
wipes local SQLite on the free tier. This page makes that a 10-second
