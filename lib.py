...
# ---------- Backup tracking ----------
#
# The app's data lives in one local SQLite file, which is NOT preserved
# across redeploys on Streamlit Cloud's free tier — a lesson learned the
# hard way. These functions don't do the backup themselves (that lives in
# the Backup & Restore page, since it needs Streamlit's download button),
# but they track *when* one last happened, so the app can nag you if it's
# been too long.

def record_backup(method):
    with get_conn() as conn:
        conn.execute(
            "INSERT INTO backup_log (backup_date, method) VALUES (?,?)",
            (now_iso(), method),
        )


def last_backup():
    # Ordered by id, not backup_date: two backups taken within the same
    # second would otherwise tie on the timestamp (it's second-precision)
    # and the ordering isn't guaranteed — id is always insertion order.
    with get_conn() as conn:
        row = conn.execute(
...
def days_since_backup():
    lb = last_backup()
    if not lb:
        return None  # never backed up
    last_dt = datetime.fromisoformat(lb["backup_date"])
    return (datetime.now() - last_dt).days


def database_row_counts():
    """Quick summary of what's in the database, shown on the Backup page
    so you can eyeball that a restore actually brought everything back."""
    tables = ["schedule", "inventory", "experiments", "experiment_items",
              "tracker_log", "tracker_items", "entries"]
    counts = {}
    with get_conn() as conn:
        for t in tables:
