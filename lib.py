...
def add_note(note_date, note_type, title, body):
    with get_conn() as conn:
        cur = conn.execute(
            "INSERT INTO notes (note_date, note_type, title, body, created_at) VALUES (?,?,?,?,?)",
            (note_date, note_type, title, body, now_iso()),
        )
        return cur.lastrowid


def get_notes(search=None, note_type=None, date_from=None, date_to=None, limit=500):
    ...
    if date_to:
        query += " AND note_date <= ?"
        params.append(date_to)
    query += " ORDER BY note_date DESC, id DESC LIMIT ?"
    params.append(limit)
    with get_conn() as conn:
        rows = conn.execute(query, params).fetchall()
    return [dict(r) for r in rows]
...
def notes_calendar_days(year, month):
    """Set of ISO date strings within the given month that have at least one note."""
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT DISTINCT note_date FROM notes WHERE note_date LIKE ?",
...
def add_followup(title, category, description, due_date):
    with get_conn() as conn:
        cur = conn.execute(
            "INSERT INTO followups (title, category, description, created_date, due_date, status, last_touched) "
            "VALUES (?,?,?,?,?,?,?)",
            (title, category, description, today_iso(), due_date, "open", today_iso()),
        )
        return cur.lastrowid


def get_followups(status="open"):
    with get_conn() as conn:
        if status == "all":
            rows = conn.execute("SELECT * FROM followups ORDER BY due_date IS NULL, due_date, id").fetchall()
        else:
            rows = conn.execute(
                "SELECT * FROM followups WHERE status=? ORDER BY due_date IS NULL, due_date, id",
                (status,),
            ).fetchall()
    return [dict(r) for r in rows]


def mark_followup(followup_id, status):
    with get_conn() as conn:
        conn.execute(
            "UPDATE followups SET status=?, last_touched=? WHERE id=?",
            (status, today_iso(), followup_id),
        )


def snooze_followup(followup_id, new_due_date):
    with get_conn() as conn:
        conn.execute(
            "UPDATE followups SET due_date=?, last_touched=? WHERE id=?",
            (new_due_date, today_iso(), followup_id),
        )
...
def overdue_followups():
    ...
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT * FROM followups WHERE status='open' AND due_date IS NOT NULL AND due_date < ? "
